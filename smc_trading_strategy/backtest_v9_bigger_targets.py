"""
V9: Bigger Targets - V8 + увеличенные TP и trailing

ИЗМЕНЕНИЯ vs V8:
✅ Увеличены TP (+17-20%):
   - LONG TREND: 30/55/90п → 35/65/100п
   - LONG RANGE: 20/35/50п → 25/45/65п
   - SHORT TREND: 15/25/35п → 18/30/42п

✅ Увеличен trailing (+30-40%):
   - LONG TREND: 18п → 25п (2.6x средний range)
   - LONG RANGE: 15п → 20п
   - SHORT TREND: 10п → 13п

ПРОБЛЕМА V8:
- TP3 (90п) слишком далеко - только 2.9% сделок достигают
- Trailing (18п) слишком близко - 1.9x средний range (срабатывает на откатах)
- TP3 = 3.7x среднее 6ч движение - нереалистично высоко

АНАЛИЗ ДВИЖЕНИЙ ЗОЛОТА:
- Средний range 1H: 9.5п
- Среднее движение 12ч: 35.6п
- 90% 12ч движений: 66п
- Наш старый TP3 (90п) покрывал только экстремальные движения

РЕШЕНИЕ V9:
- TP3 100п = ближе к 90% 12ч движений (66п)
- Trailing 25п = 2.6x средний range (больше пространства для колебаний)
- Умеренное увеличение (+20%), не агрессивное как V4 (+30%)

ОСТАВЛЕНО БЕЗ ИЗМЕНЕНИЙ:
- Partial close: 30%/30%/40% (работает отлично)
- Timeout: 60ч/48ч/24ч (оптимальные из V7 анализа)
- Расширенный SL (2-й swing)
- Market regime detection

ОЖИДАЕМЫЙ РЕЗУЛЬТАТ:
- V8: +47.25% PnL
- Прогноз V9: +52-58% PnL (больше TP hits, меньше ранних TRAILING)
"""
...
class BiggerTargetsBacktestV9:
    """V9: V8 parameters + bigger TP (35/65/100) and trailing (25п)"""
    ...
    def __init__(self, spread_points=2.0, commission_points=0.5, swap_per_day=-0.3):
        ...
        # LONG TREND parameters - INCREASED (+17-20%)
        ...
        # LONG RANGE parameters - INCREASED (+25-30%)
        ...
        # SHORT TREND parameters - INCREASED (+20%)
    ...
    def backtest(self, df, strategy, close_pct1=0.3, close_pct2=0.3, close_pct3=0.4):
        ...
        print(f"\n{'='*80}")
        print(f"📊 V9: BIGGER TARGETS (V8 + Increased TP/Trailing)")
        print(f"{'='*80}")
        print(f"   Data: {len(df)} candles")
        print(f"   Period: {df.index[0]} to {df.index[-1]}")

        print(f"\n   🎯 LONG PARAMETERS (INCREASED +17-30%):")
        print(f"   TREND: TP {self.long_trend_tp1}/{self.long_trend_tp2}/{self.long_trend_tp3}п (was 30/55/90), Trailing {self.long_trend_trailing}п (was 18), Timeout {self.long_trend_timeout}ч")
        print(f"   RANGE: TP {self.long_range_tp1}/{self.long_range_tp2}/{self.long_range_tp3}п (was 20/35/50), Trailing {self.long_range_trailing}п (was 15), Timeout {self.long_range_timeout}ч")

        print(f"\n   📉 SHORT PARAMETERS (INCREASED +20%):")
        print(f"   TREND: TP {self.short_trend_tp1}/{self.short_trend_tp2}/{self.short_trend_tp3}п (was 15/25/35), Trailing {self.short_trend_trailing}п (was 10), Timeout {self.short_trend_timeout}ч")
        print(f"   RANGE: {'DISABLED' if not self.short_range_enabled else 'ENABLED'}")

        print(f"\n   ✨ PARTIAL CLOSE (from V8):")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='V9 Backtest: Bigger Targets (TP 35/65/100, Trailing 25п)')
    ...
    # Run V9 backtest with bigger targets
    backtest = BiggerTargetsBacktestV9()
    trades_df = backtest.backtest(df, strategy)

    if trades_df is not None:
        trades_df.to_csv('backtest_v9_bigger_targets_results.csv', index=False)
        print(f"\n💾 Results saved to backtest_v9_bigger_targets_results.csv")
