"""
V10: Conservative Targets - меньшие TP 15/30/50, быстрый trailing 10п

ИЗМЕНЕНИЯ vs V9:
✅ СИЛЬНО уменьшены TP:
   - Все режимы: 15/30/50п (uniform)
   - LONG TREND: было 35/65/100п → 15/30/50п (-57%/-54%/-50%)
   - LONG RANGE: было 25/45/65п → 15/30/50п (-40%/-33%/-23%)
   - SHORT TREND: было 18/30/42п → 15/30/50п

✅ СИЛЬНО уменьшен trailing:
   - Все режимы: 10п (uniform)
   - LONG TREND: было 25п → 10п (-60%)
   - LONG RANGE: было 20п → 10п (-50%)
   - SHORT TREND: было 13п → 10п (-23%)

ГИПОТЕЗА:
- Меньшие TP = больше хитов TP1/TP2/TP3
- Быстрый trailing 10п = быстрее фиксируем прибыль
- Меньше timeout (сделки закроются раньше)
- Стратегия: Много маленьких побед вместо редких больших

АНАЛИЗ РЫНКА:
- Средний range 1H: 9.5п → trailing 10п = 1.05x range
- Среднее 3ч движение: 16.9п → TP1 15п = 89% от avg 3ч
- TP2 30п = 122% от avg 3ч движения
- TP3 50п = 203% от avg 3ч движения

РИСКИ:
⚠️ Trailing 10п может срабатывать на шуме
⚠️ TP слишком близко - можем потерять большие движения
⚠️ V9 показал что больший trailing уменьшает timeout потери

ОЖИДАЕМЫЙ РЕЗУЛЬТАТ:
- V9: +49.33% PnL (63.2% WR, 337 trades)
- Прогноз V10: либо +60%+ (много хитов), либо хуже (ранние выходы)
"""
...
class ConservativeBacktestV10:
    """V10: Conservative targets with uniform TP (15/30/50) and trailing (10п)"""
    ...
    def __init__(self, spread_points=2.0, commission_points=0.5, swap_per_day=-0.3):
        ...
        # LONG TREND parameters - CONSERVATIVE (uniform 15/30/50)
        self.long_trend_tp1 = 15  # V9 was 35 (-57%)
        self.long_trend_tp2 = 30  # V9 was 65 (-54%)
        self.long_trend_tp3 = 50  # V9 was 100 (-50%)
        self.long_trend_trailing = 10  # V9 was 25 (-60%)
        self.long_trend_timeout = 60

        # LONG RANGE parameters - CONSERVATIVE (uniform 15/30/50)
        self.long_range_tp1 = 15  # V9 was 25 (-40%)
        self.long_range_tp2 = 30  # V9 was 45 (-33%)
        self.long_range_tp3 = 50  # V9 was 65 (-23%)
        self.long_range_trailing = 10  # V9 was 20 (-50%)
        self.long_range_timeout = 48

        # SHORT TREND parameters - CONSERVATIVE (uniform 15/30/50)
        self.short_trend_tp1 = 15  # V9 was 18 (-17%)
        self.short_trend_tp2 = 30  # V9 was 30 (same)
        self.short_trend_tp3 = 50  # V9 was 42 (+19%)
        self.short_trend_trailing = 10  # V9 was 13 (-23%)
    ...
    def backtest(self, df, strategy, close_pct1=0.3, close_pct2=0.3, close_pct3=0.4):
        """Run V9 backtest with bigger TP and trailing"""

        print(f"\n{'='*80}")
        print(f"📊 V10: CONSERVATIVE TARGETS (Uniform 15/30/50, Trailing 10п)")
        print(f"{'='*80}")
        print(f"   Data: {len(df)} candles")
        print(f"   Period: {df.index[0]} to {df.index[-1]}")

        print(f"\n   🎯 ALL PARAMETERS (UNIFORM CONSERVATIVE):")
        print(f"   TP: {self.long_trend_tp1}/{self.long_trend_tp2}/{self.long_trend_tp3}п (all modes)")
        print(f"   Trailing: {self.long_trend_trailing}п (all modes)")
        print(f"   V9 was: TREND 35/65/100п/25п, RANGE 25/45/65п/20п")
        print(f"   Timeout: TREND {self.long_trend_timeout}ч, RANGE {self.long_range_timeout}ч, SHORT {self.short_trend_timeout}ч")

        print(f"\n   ✨ PARTIAL CLOSE (from V8/V9):")
        print(f"   {int(close_pct1*100)}% на TP1, {int(close_pct2*100)}% на TP2, {int(close_pct3*100)}% на TP3")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='V10 Backtest: Conservative Targets (Uniform TP 15/30/50, Trailing 10п)')
    ...
    # Run V10 backtest with conservative targets
    backtest = ConservativeBacktestV10()
    trades_df = backtest.backtest(df, strategy)

    if trades_df is not None:
        trades_df.to_csv('backtest_v10_conservative_results.csv', index=False)
        print(f"\n💾 Results saved to backtest_v10_conservative_results.csv")
