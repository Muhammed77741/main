"""
Test All Strategies on YEARLY XAUUSD Real Data (365 days)
Final validation on full year of market data
"""
...
def load_yearly_xauusd_data(file_path='XAUUSD_1H_20251227_220725.csv'):
    """
    Load yearly XAUUSD data from CSV

    Expected format:
    ,Close,Open,High,Low,Volume
    2024-12-27 22:07:25.355486,1796.413,1794.616,1796.947,1793.977,240353
    """
    print("=" * 70)
    print("LOADING YEARLY XAUUSD DATA (1 YEAR)")
    print("=" * 70)

    # Read CSV - index is in first column (unnamed)
    ...
    # Rename columns to standard format (CSV has Close,Open,High,Low - need Open,High,Low,Close)
    ...
    print(f"   Total move: ${df['close'].iloc[-1] - df['close'].iloc[0]:.2f} ({(df['close'].iloc[-1] / df['close'].iloc[0] - 1) * 100:.1f}%)")
...
def test_strategy(strategy, df, strategy_name):
    """Test a strategy on yearly data"""
    print(f"\n{'='*70}")
    ...
    try:
        ...
        if total_months > 0:
            ...
            print(f"   Monthly PnL: {total_pnl/total_months:.2f}% avg per month")
        ...
        return {
            ...
            'monthly_pnl': total_pnl / total_months if total_months > 0 else 0,
...
def compare_results(results_list):
    ...
    print(f"\n{'='*70}")
    print("FINAL COMPARISON - YEARLY REAL DATA")
    print(f"{'='*70}")
    ...
    print(f"\n{'Rank':<6} {'Strategy':<35} {'PnL':<12} {'WR':<8} {'Trades':<10} {'Monthly':<10} {'Sharpe':<8}")
    print("-" * 95)
    ...
    for i, result in enumerate(valid_results):
        ...
        print(f"{medal}  {i+1}  {result['name']:<35} {result['total_pnl']:+8.2f}%  "
              f"{result['win_rate']:5.1f}%  {result['total_trades']:6} ({result['trades_per_month']:4.1f}/mo)  "
              f"{result['monthly_pnl']:+7.2f}%  {result['sharpe']:6.2f}")

    # Winner
    winner = valid_results[0]
    print(f"\n🏆 WINNER: {winner['name']}")
    print(f"   Total PnL: {winner['total_pnl']:+.2f}% over {winner['total_months']} months")
    print(f"   Monthly Avg: {winner['monthly_pnl']:+.2f}%")
    print(f"   Win Rate: {winner['win_rate']:.1f}%")
    print(f"   Trades: {winner['total_trades']} ({winner['trades_per_month']:.1f}/month)")
...
def plot_comparison(results_list, df):
    ...
    ax1.set_title('Equity Curves - YEARLY REAL DATA (1 Year)', fontsize=14, fontweight='bold')
    ax1.set_xlabel('Date')
    ...
    ax2.set_title('Strategy Performance - YEARLY DATA', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('yearly_data_comparison.png', dpi=150, bbox_inches='tight')
    print(f"\n💾 Chart saved: yearly_data_comparison.png")
    plt.close()


def main():
    # Load yearly data
    df = load_yearly_xauusd_data('/home/user/main/XAUUSD_1H_20251227_220725.csv')
    ...
    if results:
        ...
        with open('yearly_data_results.txt', 'w') as f:
            f.write("="*70 + "\n")
            f.write("YEARLY REAL DATA BACKTEST RESULTS (1 YEAR)\n")
            f.write("="*70 + "\n\n")
            ...
            for i, result in enumerate(final_results):
                ...
                f.write(f"   Monthly Avg PnL: {result['monthly_pnl']:+.2f}%\n")
        ...
        print(f"✅ Results saved to: yearly_data_results.txt")

        print(f"\n{'='*70}")
        print("✅ YEARLY DATA BACKTEST COMPLETE!")
        print(f"{'='*70}")
