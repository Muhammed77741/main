"""
Realistic Backtest V3 - MULTIPLE POSITIONS + AVERAGING + TRAILING STOP
- Multiple positions allowed
- Position averaging (доливка) if signal in same direction
- Trailing stop after TP1 (instead of breakeven)
- Lower TP levels: 20/35/50 (instead of 30/50/80)
- All V2 fixes: spread, commission, swap, daily limits
"""

import pandas as pd
import numpy as np
from datetime import datetime
import argparse

from pattern_recognition_strategy import PatternRecognitionStrategy


class RealisticBacktestV3:
    """Realistic backtest with multiple positions and averaging"""

    def __init__(self, spread_points=2.0, commission_points=0.5, swap_per_day=-0.3,
                 trailing_distance=15):
        """
        Args:
            spread_points: Average spread in points (default: 2 for XAUUSD)
            commission_points: Commission per trade in points (default: 0.5)
            swap_per_day: Swap cost per day in points (default: -0.3)
            trailing_distance: Trailing stop distance in points after TP1 (default: 15)
        """
        self.spread = spread_points
        self.commission = commission_points
        self.swap_per_day = swap_per_day
        self.trailing_distance = trailing_distance

        # Daily limits
        self.max_trades_per_day = 10  # Increased for multiple positions
        self.max_loss_per_day = -5.0  # %
        self.max_positions = 5  # Maximum 5 positions at once
        self.allow_averaging = False  # DISABLE averaging to allow more independent positions
        self.max_averaging_times = 2  # Maximum 2 times averaging per position (if enabled)
        self.max_consecutive_losses = 3

    def backtest(self, df, strategy, tp1=20, tp2=35, tp3=50,
                 close_pct1=0.5, close_pct2=0.3, close_pct3=0.2):
        """
        Run realistic backtest with multiple positions

        Args:
            df: DataFrame with OHLCV data
            strategy: Strategy instance
            tp1, tp2, tp3: Take profit levels in points (DEFAULT: 20/35/50)
            close_pct1, close_pct2, close_pct3: Partial close percentages
        """

        print(f"\n{'='*80}")
        print(f"📊 REALISTIC BACKTEST V3 - MULTIPLE POSITIONS + TRAILING STOP")
        print(f"{'='*80}")
        print(f"   Data: {len(df)} candles")
        print(f"   Period: {df.index[0]} to {df.index[-1]}")
        print(f"   TP Levels: {tp1}п / {tp2}п / {tp3}п (REDUCED!)")
        print(f"   Close %: {close_pct1*100:.0f}% / {close_pct2*100:.0f}% / {close_pct3*100:.0f}%")
        print(f"\n   💰 REALISTIC COSTS:")
        print(f"   Spread: {self.spread}п")
        print(f"   Commission: {self.commission}п")
        print(f"   Swap: {self.swap_per_day}п/day")
        print(f"\n   🛡️ RISK LIMITS:")
        print(f"   Max positions: {self.max_positions}")
        print(f"   Max trades/day: {self.max_trades_per_day}")
        print(f"   Max loss/day: {self.max_loss_per_day}%")
        print(f"\n   📈 NEW FEATURES:")
        print(f"   ✅ Multiple positions allowed (max {self.max_positions})")
        if self.allow_averaging:
            print(f"   ✅ Position averaging (доливка) - max {self.max_averaging_times} times")
        else:
            print(f"   ❌ Position averaging DISABLED (independent positions only)")
        print(f"   ✅ Trailing stop after TP1: {self.trailing_distance}п")

        # Run strategy
        df_strategy = strategy.run_strategy(df.copy())
        df_signals = df_strategy[df_strategy['signal'] != 0].copy()

        print(f"\n📈 Raw signals found: {len(df_signals)}")

        if len(df_signals) == 0:
            print("❌ No signals")
            return None

        # Track trades and open positions
        trades = []
        open_positions = []  # List of open positions

        for idx, signal in df_signals.iterrows():
            # Check daily limits
            current_date = idx.date()
            today_trades = [t for t in trades if t['exit_time'].date() == current_date]

            if len(today_trades) >= self.max_trades_per_day:
                continue

            daily_pnl = sum([t['pnl_pct'] for t in today_trades])
            if daily_pnl <= self.max_loss_per_day:
                continue

            # Check consecutive losses
            if len(trades) >= 5:
                last_5 = trades[-5:]
                if all(t['pnl_pct'] < 0 for t in last_5):
                    continue

            # Check if we can open new position
            direction = 'LONG' if signal['signal'] == 1 else 'SHORT'

            # Check if we have position in same direction (for averaging)
            same_direction_pos = [p for p in open_positions if p['direction'] == direction]

            if len(same_direction_pos) > 0 and self.allow_averaging:
                # AVERAGING: Add to existing position (with limit!)
                pos = same_direction_pos[0]  # Take first position in same direction

                # Check averaging limit
                if pos.get('averaged_times', 0) >= self.max_averaging_times:
                    continue  # Skip if already averaged max times

                entry_time = idx
                close_price = signal['entry_price']
                signal_sl = signal['stop_loss']

                if direction == 'LONG':
                    new_entry = close_price + self.spread / 2
                else:
                    new_entry = close_price - self.spread / 2

                # Average entry price
                old_size = pos.get('position_size', 1.0)
                new_size = 1.0  # Add 1 lot
                total_size = old_size + new_size

                averaged_entry = (pos['entry_price'] * old_size + new_entry * new_size) / total_size

                # Update position
                pos['entry_price'] = averaged_entry
                pos['position_size'] = total_size

                # Recalculate TPs from averaged entry
                if direction == 'LONG':
                    pos['tp1_price'] = averaged_entry + tp1
                    pos['tp2_price'] = averaged_entry + tp2
                    pos['tp3_price'] = averaged_entry + tp3
                else:
                    pos['tp1_price'] = averaged_entry - tp1
                    pos['tp2_price'] = averaged_entry - tp2
                    pos['tp3_price'] = averaged_entry - tp3

                pos['averaged_times'] = pos.get('averaged_times', 0) + 1

                print(f"📊 AVERAGING: {direction} position @ {averaged_entry:.2f} (size: {total_size:.1f})")
                continue

            # Check max positions limit
            if len(open_positions) >= self.max_positions:
                continue

            # Open NEW position
            entry_time = idx
            close_price = signal['entry_price']
            signal_sl = signal['stop_loss']

            if direction == 'LONG':
                entry_price = close_price + self.spread / 2
                sl_distance = close_price - signal_sl
                sl_price = entry_price - sl_distance
            else:
                entry_price = close_price - self.spread / 2
                sl_distance = signal_sl - close_price
                sl_price = entry_price + sl_distance

            # Calculate TPs
            if direction == 'LONG':
                tp1_price = entry_price + tp1
                tp2_price = entry_price + tp2
                tp3_price = entry_price + tp3
            else:
                tp1_price = entry_price - tp1
                tp2_price = entry_price - tp2
                tp3_price = entry_price - tp3

            new_position = {
                'entry_time': entry_time,
                'entry_price': entry_price,
                'direction': direction,
                'sl_price': sl_price,
                'original_sl': sl_price,
                'tp1_price': tp1_price,
                'tp2_price': tp2_price,
                'tp3_price': tp3_price,
                'tp1_hit': False,
                'tp2_hit': False,
                'tp3_hit': False,
                'trailing_active': False,  # NEW: Trailing stop activated after TP1
                'trailing_high': 0,  # Track highest price for trailing
                'trailing_low': 0,   # Track lowest price for trailing
                'position_remaining': 1.0,
                'position_size': 1.0,  # Track size for averaging
                'total_pnl_pct': 0.0,
                'averaged_times': 0,
                'pattern': signal.get('pattern', 'N/A')
            }

            open_positions.append(new_position)

        # Simulate positions through time
        for i in range(len(df)):
            candle = df.iloc[i]
            candle_time = df.index[i]
            high = candle['high']
            low = candle['low']
            close = candle['close']

            positions_to_close = []

            for pos_idx, pos in enumerate(open_positions):
                entry_time = pos['entry_time']
                entry_price = pos['entry_price']
                direction = pos['direction']

                # Check timeout (48 hours)
                hours_in_trade = (candle_time - entry_time).total_seconds() / 3600

                if hours_in_trade >= 48:
                    # Force close
                    if direction == 'LONG':
                        exit_price = close - self.spread / 2
                    else:
                        exit_price = close + self.spread / 2

                    pnl_points = self._calculate_pnl_points(
                        entry_price, exit_price, direction,
                        pos['position_remaining'], hours_in_trade,
                        pos.get('position_size', 1.0)
                    )

                    pos['total_pnl_pct'] += (pnl_points / entry_price) * 100

                    trade = self._create_trade_record(pos, exit_price, 'TIMEOUT', candle_time)
                    trades.append(trade)
                    positions_to_close.append(pos_idx)
                    continue

                # Check SL/TP
                if direction == 'LONG':
                    # Update trailing high for trailing stop
                    if pos['trailing_active']:
                        if high > pos['trailing_high']:
                            pos['trailing_high'] = high
                            # Move trailing stop
                            new_trailing_sl = high - self.trailing_distance
                            if new_trailing_sl > pos['sl_price']:
                                pos['sl_price'] = new_trailing_sl

                    # Stop loss
                    if low <= pos['sl_price']:
                        exit_price = pos['sl_price']

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            pos['position_remaining'], hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100

                        exit_type = 'TRAILING_SL' if pos['trailing_active'] else 'SL'
                        trade = self._create_trade_record(pos, exit_price, exit_type, candle_time)
                        trades.append(trade)
                        positions_to_close.append(pos_idx)
                        continue

                    # TP1
                    if high >= pos['tp1_price'] and not pos['tp1_hit']:
                        exit_price = pos['tp1_price'] - self.spread / 2

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            close_pct1, hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100
                        pos['position_remaining'] -= close_pct1
                        pos['tp1_hit'] = True

                        # ACTIVATE TRAILING STOP (instead of breakeven)
                        pos['trailing_active'] = True
                        pos['trailing_high'] = high
                        pos['sl_price'] = high - self.trailing_distance

                    # TP2
                    if high >= pos['tp2_price'] and not pos['tp2_hit']:
                        exit_price = pos['tp2_price'] - self.spread / 2

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            close_pct2, hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100
                        pos['position_remaining'] -= close_pct2
                        pos['tp2_hit'] = True

                    # TP3
                    if high >= pos['tp3_price'] and not pos['tp3_hit']:
                        exit_price = pos['tp3_price'] - self.spread / 2

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            close_pct3, hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100
                        pos['position_remaining'] -= close_pct3
                        pos['tp3_hit'] = True

                    # Close if all TPs hit
                    if pos['position_remaining'] <= 0.01:
                        trade = self._create_trade_record(pos, pos['tp3_price'], 'TP3', candle_time)
                        trades.append(trade)
                        positions_to_close.append(pos_idx)
                        continue

                else:  # SHORT
                    # Update trailing low for trailing stop
                    if pos['trailing_active']:
                        if low < pos['trailing_low'] or pos['trailing_low'] == 0:
                            pos['trailing_low'] = low
                            # Move trailing stop
                            new_trailing_sl = low + self.trailing_distance
                            if new_trailing_sl < pos['sl_price']:
                                pos['sl_price'] = new_trailing_sl

                    # Stop loss
                    if high >= pos['sl_price']:
                        exit_price = pos['sl_price']

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            pos['position_remaining'], hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100

                        exit_type = 'TRAILING_SL' if pos['trailing_active'] else 'SL'
                        trade = self._create_trade_record(pos, exit_price, exit_type, candle_time)
                        trades.append(trade)
                        positions_to_close.append(pos_idx)
                        continue

                    # TP1
                    if low <= pos['tp1_price'] and not pos['tp1_hit']:
                        exit_price = pos['tp1_price'] + self.spread / 2

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            close_pct1, hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100
                        pos['position_remaining'] -= close_pct1
                        pos['tp1_hit'] = True

                        # ACTIVATE TRAILING STOP
                        pos['trailing_active'] = True
                        pos['trailing_low'] = low
                        pos['sl_price'] = low + self.trailing_distance

                    # TP2
                    if low <= pos['tp2_price'] and not pos['tp2_hit']:
                        exit_price = pos['tp2_price'] + self.spread / 2

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            close_pct2, hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100
                        pos['position_remaining'] -= close_pct2
                        pos['tp2_hit'] = True

                    # TP3
                    if low <= pos['tp3_price'] and not pos['tp3_hit']:
                        exit_price = pos['tp3_price'] + self.spread / 2

                        pnl_points = self._calculate_pnl_points(
                            entry_price, exit_price, direction,
                            close_pct3, hours_in_trade,
                            pos.get('position_size', 1.0)
                        )

                        pos['total_pnl_pct'] += (pnl_points / entry_price) * 100
                        pos['position_remaining'] -= close_pct3
                        pos['tp3_hit'] = True

                    # Close if all TPs hit
                    if pos['position_remaining'] <= 0.01:
                        trade = self._create_trade_record(pos, pos['tp3_price'], 'TP3', candle_time)
                        trades.append(trade)
                        positions_to_close.append(pos_idx)
                        continue

            # Remove closed positions
            for idx in sorted(positions_to_close, reverse=True):
                del open_positions[idx]

        # Convert to DataFrame
        if len(trades) == 0:
            print("❌ No completed trades")
            return None

        trades_df = pd.DataFrame(trades)

        # Calculate statistics
        self._print_results(trades_df)

        return trades_df

    def _calculate_pnl_points(self, entry, exit, direction, position_size, hours, lot_size=1.0):
        """Calculate PnL in points with all costs"""

        # Price movement
        if direction == 'LONG':
            price_pnl = exit - entry
        else:
            price_pnl = entry - exit

        # Apply position size and lot size
        price_pnl *= position_size * lot_size

        # Subtract costs
        price_pnl -= self.commission * position_size * lot_size

        # Subtract swap if held >24h
        if hours > 24:
            days = hours / 24
            price_pnl += self.swap_per_day * days * position_size * lot_size

        return price_pnl

    def _create_trade_record(self, pos, exit_price, exit_type, exit_time):
        """Create trade record"""

        duration_hours = (exit_time - pos['entry_time']).total_seconds() / 3600

        pnl_pct = pos['total_pnl_pct']
        pnl_points = (pnl_pct / 100) * pos['entry_price']

        return {
            'entry_time': pos['entry_time'],
            'exit_time': exit_time,
            'direction': pos['direction'],
            'entry_price': pos['entry_price'],
            'exit_price': exit_price,
            'stop_loss': pos['original_sl'],
            'position_size': pos.get('position_size', 1.0),
            'averaged_times': pos.get('averaged_times', 0),
            'tp1_hit': pos['tp1_hit'],
            'tp2_hit': pos['tp2_hit'],
            'tp3_hit': pos['tp3_hit'],
            'trailing_used': pos['trailing_active'],
            'exit_type': exit_type,
            'pnl_pct': pnl_pct,
            'pnl_points': pnl_points,
            'duration_hours': duration_hours,
            'pattern': pos['pattern']
        }

    def _print_results(self, trades_df):
        """Print backtest results"""

        print(f"\n{'='*80}")
        print(f"📊 REALISTIC RESULTS V3 - MULTIPLE POSITIONS + TRAILING STOP")
        print(f"{'='*80}")

        total_trades = len(trades_df)
        wins = len(trades_df[trades_df['pnl_pct'] > 0])
        losses = total_trades - wins
        win_rate = (wins / total_trades * 100) if total_trades > 0 else 0

        total_pnl = trades_df['pnl_pct'].sum()
        total_points = trades_df['pnl_points'].sum()

        avg_win = trades_df[trades_df['pnl_pct'] > 0]['pnl_pct'].mean() if wins > 0 else 0
        avg_loss = trades_df[trades_df['pnl_pct'] <= 0]['pnl_pct'].mean() if losses > 0 else 0

        # TP hit rates
        tp1_hits = trades_df['tp1_hit'].sum()
        tp2_hits = trades_df['tp2_hit'].sum()
        tp3_hits = trades_df['tp3_hit'].sum()
        trailing_used = trades_df['trailing_used'].sum()

        # Averaging stats
        averaged_trades = len(trades_df[trades_df['averaged_times'] > 0])
        avg_times = trades_df[trades_df['averaged_times'] > 0]['averaged_times'].mean() if averaged_trades > 0 else 0

        print(f"\n📈 Performance:")
        print(f"   Total trades: {total_trades}")
        print(f"   Wins: {wins} ({win_rate:.1f}%)")
        print(f"   Losses: {losses}")
        print(f"   Total PnL: {total_pnl:+.2f}%")
        print(f"   Total Points: {total_points:+.1f}п")
        print(f"   Avg Win: {avg_win:+.2f}%")
        print(f"   Avg Loss: {avg_loss:+.2f}%")

        if avg_loss != 0:
            profit_factor = abs(avg_win * wins / (avg_loss * losses))
            print(f"   Profit Factor: {profit_factor:.2f}")

        print(f"\n🎯 TP Hit Rates:")
        print(f"   TP1: {tp1_hits}/{total_trades} ({tp1_hits/total_trades*100:.1f}%)")
        print(f"   TP2: {tp2_hits}/{total_trades} ({tp2_hits/total_trades*100:.1f}%)")
        print(f"   TP3: {tp3_hits}/{total_trades} ({tp3_hits/total_trades*100:.1f}%)")
        print(f"   Trailing stop used: {trailing_used}/{total_trades} ({trailing_used/total_trades*100:.1f}%)")

        print(f"\n📊 Averaging Stats:")
        print(f"   Positions with averaging: {averaged_trades}/{total_trades} ({averaged_trades/total_trades*100:.1f}%)")
        if averaged_trades > 0:
            print(f"   Avg times averaged: {avg_times:.1f}")

        # Exit types
        print(f"\n🚪 Exit Types:")
        for exit_type in trades_df['exit_type'].unique():
            count = len(trades_df[trades_df['exit_type'] == exit_type])
            print(f"   {exit_type}: {count} ({count/total_trades*100:.1f}%)")

        # Drawdown
        cumulative_pnl = trades_df['pnl_pct'].cumsum()
        running_max = cumulative_pnl.cummax()
        drawdown = cumulative_pnl - running_max
        max_drawdown = drawdown.min()

        print(f"\n📉 Risk Metrics:")
        print(f"   Max Drawdown: {max_drawdown:.2f}%")

        # Monthly
        trades_df['month'] = pd.to_datetime(trades_df['exit_time']).dt.to_period('M')
        monthly = trades_df.groupby('month').agg({
            'pnl_pct': 'sum',
            'entry_time': 'count'
        }).rename(columns={'entry_time': 'trades'})

        print(f"\n📅 Monthly Results:")
        print(monthly)


def main():
    """Main entry point"""

    parser = argparse.ArgumentParser(description='Realistic Backtest V3 - Multiple Positions + Trailing Stop')

    parser.add_argument('--file', type=str, required=True,
                        help='CSV file with H1 data')

    parser.add_argument('--spread', type=float, default=2.0,
                        help='Spread in points (default: 2)')

    parser.add_argument('--commission', type=float, default=0.5,
                        help='Commission in points (default: 0.5)')

    parser.add_argument('--swap', type=float, default=-0.3,
                        help='Swap per day in points (default: -0.3)')

    parser.add_argument('--trailing', type=float, default=15,
                        help='Trailing stop distance in points (default: 15)')

    args = parser.parse_args()

    # Load data
    print(f"\n📂 Loading data from {args.file}...")
    df = pd.read_csv(args.file)
    df['datetime'] = pd.to_datetime(df['datetime'])
    df = df.set_index('datetime')

    print(f"✅ Loaded {len(df)} candles")
    print(f"   Period: {df.index[0]} to {df.index[-1]}")

    # Add market hours if not present
    if 'is_active' not in df.columns:
        df['is_london'] = df.index.hour.isin(range(7, 12))
        df['is_ny'] = df.index.hour.isin(range(13, 20))
        df['is_overlap'] = df.index.hour.isin(range(13, 16))
        df['is_active'] = df['is_london'] | df['is_ny']
        df['is_best_hours'] = df.index.hour.isin([8, 9, 10, 13, 14, 15])

    # Create strategy
    strategy = PatternRecognitionStrategy(fib_mode='standard')

    # Run realistic backtest V3
    backtest = RealisticBacktestV3(
        spread_points=args.spread,
        commission_points=args.commission,
        swap_per_day=args.swap,
        trailing_distance=args.trailing
    )

    trades_df = backtest.backtest(
        df=df,
        strategy=strategy,
        tp1=20, tp2=35, tp3=50,  # REDUCED TP LEVELS!
        close_pct1=0.5, close_pct2=0.3, close_pct3=0.2
    )

    if trades_df is not None:
        # Save results
        output_file = f"backtest_v3_multiple_positions_results.csv"
        trades_df.to_csv(output_file, index=False)
        print(f"\n💾 Results saved to {output_file}")

        print(f"\n{'='*80}")
        print(f"✅ REALISTIC BACKTEST V3 COMPLETE")
        print(f"{'='*80}")
        print(f"\n🔍 Key features:")
        print(f"   ✅ Multiple positions (max {backtest.max_positions})")
        print(f"   ✅ Position averaging (доливка)")
        print(f"   ✅ Trailing stop after TP1: {args.trailing}п")
        print(f"   ✅ Lower TP levels: 20п/35п/50п")
        print(f"   ✅ Spread: {args.spread}п")
        print(f"   ✅ Commission: {args.commission}п")
        print(f"   ✅ Swap: {args.swap}п/day")


if __name__ == "__main__":
    # Example:
    # python backtest_v3_multiple_positions.py --file XAUUSD_1H_MT5_20241227_20251227.csv --trailing 15
    main()
