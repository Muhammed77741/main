"""
SHORT Optimized + Pattern Filtering V2

ИСПРАВЛЕНО:
- Pre-calculate swing points для всех данных (как в Patterns Only)
- Использовать последние 5 swing points (вместо 2)
- Более мягкие условия детекции

ФИЛЬТРЫ:
1. SHORT только в TREND (из SHORT Optimized)
2. НОВОЕ: Фильтрация плохих паттернов
...
class PatternFilteredBacktestV2:
    """SHORT Optimized + improved pattern filtering"""
    ...
    def find_swing_points(self, df):
        """
        Find all swing highs and lows in dataframe
        Returns: (swing_highs_df, swing_lows_df)
        """
        swing_highs = []
        swing_lows = []
        ...
        swing_highs_df = pd.DataFrame(swing_highs).set_index('time') if swing_highs else pd.DataFrame()
        swing_lows_df = pd.DataFrame(swing_lows).set_index('time') if swing_lows else pd.DataFrame()

        return swing_highs_df, swing_lows_df

    def detect_pattern(self, df, idx, swing_highs_df, swing_lows_df):
        """
        Detect chart pattern at index using pre-calculated swings
        """
        current_time = df.index[idx]
        current_close = df['close'].iloc[idx]

        # Get recent swings BEFORE current signal
        recent_highs = swing_highs_df[swing_highs_df.index < current_time].tail(5)
        recent_lows = swing_lows_df[swing_lows_df.index < current_time].tail(5)
    ...
    def should_filter_pattern(self, pattern):
    ...
    def backtest(self, df, strategy, close_pct1=0.5, close_pct2=0.3, close_pct3=0.2):
        ...
        print(f"\n{'='*80}")
        print(f"🎯 PATTERN-FILTERED BACKTEST V2 (improved pattern detection)")
        print(f"{'='*80}")
        ...
        if self.enable_pattern_filtering:
            ...
            print(f"   Tolerance: {self.pattern_tolerance*100}%")

        # Find ALL swing points first
        print(f"\n   🔍 Finding swing points...")
        swing_highs_df, swing_lows_df = self.find_swing_points(df)
        print(f"   Found {len(swing_highs_df)} swing highs, {len(swing_lows_df)} swing lows")
        ...
        for i in range(len(df_strategy)):
            ...
            if signal != 0:
                ...
                # FILTER 2: PATTERN FILTERING (improved!)
                pattern = self.detect_pattern(df_strategy, i, swing_highs_df, swing_lows_df)
                should_filter, filter_reason = self.should_filter_pattern(pattern)
            ...
            # Update positions (same logic as before - copied from SHORT Optimized)
            positions_to_close = []
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Pattern-Filtered Backtest V2')
    ...
    # Run pattern-filtered backtest
    backtest = PatternFilteredBacktestV2()
    trades_df = backtest.backtest(df, strategy)

    if trades_df is not None:
        trades_df.to_csv('backtest_v3_pattern_filtered_v2_results.csv', index=False)
        print(f"\n💾 Results saved to backtest_v3_pattern_filtered_v2_results.csv")
