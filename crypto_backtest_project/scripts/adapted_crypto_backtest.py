"""
Adapted Crypto Backtest - Percentage-based parameters for BTC/ETH
Based on v3_adaptive but with parameters adapted for cryptocurrency volatility
"""
...
class AdaptedCryptoBacktest:
    """Adapted backtest with percentage-based parameters for crypto"""

    def __init__(self, spread_pct=0.05, commission_pct=0.1):
        """
        Args:
            spread_pct: Spread in percentage (default: 0.05%)
            commission_pct: Commission in percentage (default: 0.1%)
        """
        self.spread_pct = spread_pct
        self.commission_pct = commission_pct

        # TREND MODE parameters (percentage-based for crypto)
        ...
        self.trend_timeout = 72  # 72 hours (3 days)
        ...
        self.range_timeout = 48  # 48 hours (2 days)
    ...
    def detect_market_regime(self, df, current_idx, lookback=100):
        ...
        directional_bias = trend_strength > 0.2
    ...
    def backtest(self, df, strategy, close_pct1=0.5, close_pct2=0.3, close_pct3=0.2):
        """Run adapted crypto backtest"""

        print(f"\n{'='*80}")
        print(f"🪙 ADAPTED CRYPTO BACKTEST - PERCENTAGE-BASED PARAMETERS")
        print(f"{'='*80}")
        print(f"   Data: {len(df)} candles")
        print(f"   Period: {df.index[0]} to {df.index[-1]}")

        print(f"\n   📈 TREND MODE (Percentage-based):")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Adapted Crypto Backtest')
    parser.add_argument('--file', type=str, required=True, help='CSV file with crypto data')
    parser.add_argument('--output', type=str, help='Output file for results (default: auto-generate)')
    ...
    # Run backtest
    backtest = AdaptedCryptoBacktest()
    ...
    if trades_df is not None:
        ...
        if args.output:
            ...
            output_file = f'{crypto_name}_adapted_results.csv'
